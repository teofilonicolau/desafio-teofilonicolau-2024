class RecintosZoo {
  ...
  analisaRecintos(animal, quantidade) {
    ...
    const recintosViaveis = this.recintos
      ...
      .filter((recinto) =>
        this.temEspacoSuficiente(recinto, animal, quantidade)
      )
      .filter((recinto) =>
        this.validaCompatibilidadeAnimais(recinto, animal, quantidade)
      )
      .map(
        (recinto) =>
          `Recinto ${
            recinto.numero
          } (espaço livre: ${this.calculaEspacoRestante(
            recinto,
            animal,
            quantidade
  ...
  isBiomaCompatível(recinto, animal) {
    ...
    return this.animais[animal].biomas.some((bioma) =>
  ...
  validaCompatibilidadeAnimais(recinto, especie, quantidade) {
    ...
    if (animal.carnívoro) {
      if (
        animaisNoRecinto.some((a) => this.animais[a].carnívoro && a !== especie)
      ) {
        return false;
      }
